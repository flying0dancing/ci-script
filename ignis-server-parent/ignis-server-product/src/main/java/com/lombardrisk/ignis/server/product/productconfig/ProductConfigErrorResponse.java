...
public class ProductConfigErrorResponse {
    ...
    public static ErrorResponse productZipEmpty(final String productConfigName) {
    ...
    public static ErrorResponse productMissingManifest(final String productConfigName) {
        return ErrorResponse.valueOf(
                String.format(Code.PRODUCT_MISSING_MANIFEST.errorMessage, productConfigName),
                Code.PRODUCT_MISSING_MANIFEST.name());
    }

    public static ErrorResponse jsonFormatInvalid(final List<String> fileNames) {
        return ErrorResponse.valueOf(
                String.format(Code.JSON_FORMAT_INVALID.errorMessage, String.join(", ", fileNames)),
                Code.JSON_FORMAT_INVALID.name());
    }

    public static ErrorResponse productConfigExists(final String productConfigName) {
        return ErrorResponse.valueOf(
                String.format(Code.PRODUCT_CONFIG_EXISTS.errorMessage, productConfigName),
                Code.PRODUCT_CONFIG_EXISTS.name());
    }

    public static ErrorResponse tableExists(final String tableName) {
        return ErrorResponse.valueOf(
                String.format(Code.TABLE_EXISTS.errorMessage, tableName),
                Code.TABLE_EXISTS.name());
    }

    public static ErrorResponse productConfigNotValid(final ConstraintViolation<ProductConfig> violation) {
        return ErrorResponse.valueOf(
